...
**목표:** Spring Boot Kotlin 프로젝트 혼자 세팅하고 API 하나 짤 수 있는 수준
...
- [ ] Spring Boot + Kotlin Hello World 프로젝트 직접 세팅
- [ ] 기존 Java 코드 Kotlin으로 변환 연습 (간단한 CRUD)
- [ ] coroutine 기초 개념 파악 (suspend, launch) — 깊이는 나중에

**학습 완료 기준:** Kotlin으로 Spring Boot REST API 하나 혼자 짤 수 있으면 됨

---

### 📚 학습: liboqs-java + ML-DSA (2주)

**목표:** ML-DSA 키 생성 → 서명 → 검증 단위 테스트 통과
...
- [ ] liboqs 공식 Docker 이미지 pull
  ```bash
  docker pull openquantumsafe/liboqs
  ```
- [ ] liboqs-java 네이티브 빌드 Dockerfile 작성
- [ ] Spring Boot 프로젝트에 liboqs-java 의존성 연동
- [ ] ML-DSA 키쌍 생성 코드 작성
- [ ] ML-DSA 서명 코드 작성
- [ ] ML-DSA 검증 코드 작성
...
**목표:** PDF 파일에 ML-DSA 서명값 삽입/추출 성공

- [ ] PDFBox 기본 구조 파악 (PDDocument, PDPage 등)
- [ ] PDF 메타데이터 구조 이해
- [ ] PDFBox 의존성 추가 (Gradle)
- [ ] PDF 파일 열기 / 저장 기본 코드 작성
- [ ] ML-DSA 서명값 Base64 인코딩 후 PDF 메타데이터에 삽입
- [ ] PDF에서 서명값 추출 및 디코딩
- [ ] 삽입 → 추출 → 검증 연동 테스트

**학습 완료 기준:** PDF에 서명값 넣고 꺼내서 ML-DSA 검증 통과

---

### 🛠 개발: 로컬 환경 구성

- [ ] Gradle Kotlin DSL 기반 Spring Boot 3.2 프로젝트 생성
- [ ] Logback JSON 구조화 로그 설정 (30분이면 됨, 지금 바로)
  ```xml
  <!-- logback-spring.xml -->
  <!-- JSON 포맷 출력 설정 -->
  ```
...
### 📚 학습: Vue 3 Composition API + Pinia
...
- [ ] Pinia 상태관리
  ...
  - [ ] 컴포넌트에서 스토어 사용
- [ ] Axios 인터셉터 (JWT 자동 첨부, 401 처리)

---

### 🛠 개발: Vue 3 프론트

- [ ] Vue 3 + Vite + Pinia + Vue Router 프로젝트 세팅
- [ ] Axios 인스턴스 + JWT 인터셉터 설정
- [ ] 인증 스토어 (Pinia) — 로그인 상태 관리
- [ ] 로그인 / 회원가입 페이지
- [ ] 문서 목록 페이지
- [ ] PDF 업로드 페이지
- [ ] 서명 요청 페이지 (서명자 이메일 입력)
- [ ] 서명자 페이지 (`/sign/:token`)
  - [ ] 비회원 접근 가능
  - [ ] 이메일 OTP 인증
  - [ ] 문서 미리보기 (PDF.js 활용)
...
> Terraform 쓰기 전 콘솔로 한 번 전체를 손으로 만들어야 함
...
- [ ] `terraform plan` 으로 기존 인프라와 diff 확인
- [ ] `terraform apply` 검증
