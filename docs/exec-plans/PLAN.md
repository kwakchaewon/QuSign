...
| 2단계 | 백엔드 핵심 구현 | ✅ 완료 |
| 3단계 | 프론트엔드 + 이메일 연동 | 🔄 진행 중 (3-3·3-4 남음) |
...
- [x] **→ 즉시 적용:** 회원가입 API
  - [x] ML-DSA 키쌍 생성
  - [x] PBKDF2로 비밀번호 기반 파생 키 생성
  - [x] AES-256으로 개인키 암호화
  - [x] 암호화된 개인키 + 공개키 DB 저장
- [x] **→ 즉시 적용:** JWT 로그인 API
- [x] **→ 즉시 적용:** JWT 인증 필터 구현

---

### 2-2. 문서 처리 API 구현

- [x] 1회용 토큰 만료 처리 패턴 학습
- [x] **→ 즉시 적용:** PDF 업로드 API
  - [x] MultipartFile 수신
  - [x] SHA3-256 해시 생성
  - [x] MinIO 저장
  - [x] documents 테이블 저장
- [x] **→ 즉시 적용:** 문서 목록 조회 API
- [x] **→ 즉시 적용:** 서명된 PDF 다운로드 API
...
- [x] **→ 즉시 적용:** 서명 요청 API
  - [x] 1회용 토큰 생성 (UUID + 만료)
  - [x] signature_requests 테이블 저장
  - [x] (이메일은 3단계에서 연동, 지금은 토큰 반환만)
- [x] **→ 즉시 적용:** 서명 실행 API
  - [x] 토큰 유효성 검증 (존재 여부 + 만료 여부)
  - [x] 사용자 개인키 복호화
  - [x] ML-DSA 서명값 생성
  - [x] PDF에 서명값 삽입
  - [x] S3 저장
  - [x] signatures 테이블 기록
  - [x] 개인키 메모리에서 즉시 삭제
- [x] **→ 즉시 적용:** 무결성 검증 API (공개, 인증 불필요)
  - [x] 토큰 기반 서명 조회 → 서명값 추출
  - [x] 공개키로 ML-DSA 검증
  - [x] 결과 반환 (서명자 / 서명 시각 / documentHash / 유효 여부)
...
### 3-0. 디자인 시스템 구축 (Claude 디자인 활용)

**목표:** 화면 구현 전 디자인 시스템을 확립하고 퍼블리싱 산출물을 확보한다

- [x] 서비스 컬러 팔레트 / 타이포그래피 정의
- [x] 공통 컴포넌트 목록 정의 (버튼, 입력, 카드, 모달, 뱃지 등)
- [x] **→ 즉시 적용:** Claude 디자인으로 핵심 화면 와이어프레임 생성
  - [x] 로그인 / 회원가입 화면
  - [x] 문서 목록 화면
  - [x] PDF 업로드 / 서명 요청 화면
  - [x] 서명자 화면 (`/sign/:token`)
  - [x] 무결성 검증 화면
- [x] **→ 즉시 적용:** Claude 디자인으로 UI 목업 → HTML/CSS 퍼블리싱 산출물 생성
- [x] **→ 즉시 적용:** 디자인 토큰(CSS 변수) 정의 파일 작성
- [x] 개발자 핸드오프 — 컴포넌트별 스펙 문서 확인

**완료 기준:** 전 화면 목업 완성 + CSS 변수 파일 준비 ✅
...
- [x] Step 1 — claude.ai/design에서 전 화면 파일 다운로드
- [x] Step 2 — `npm create vue@latest frontend` 스캐폴딩
- [x] Step 3 — `tokens.css` + 각 화면 파일 배치
- [x] Step 4 — HTML → Vue SFC 변환 (화면별)
- [ ] Step 5 — `npm run dev` 브라우저 확인 ← 미완 (빌드 통과, 브라우저 렌더링 미확인)
...
- [x] **→ 즉시 적용:** Vue 3 + Vite + Pinia + Vue Router 프로젝트 세팅
- [x] **→ 즉시 적용:** Axios 인스턴스 + JWT 인터셉터 설정
- [x] **→ 즉시 적용:** 인증 스토어 (Pinia) — 로그인 상태 관리
- [x] **→ 즉시 적용:** 로그인 / 회원가입 페이지
...
- [x] 네비게이션 가드 (로그인 체크)
- [x] 동적 라우트 (`/sign/:token`)
- [x] 스토어 정의 (`defineStore`) / state, getters, actions 구조
- [x] **→ 즉시 적용:** 문서 목록 페이지
- [x] **→ 즉시 적용:** PDF 업로드 페이지
- [x] **→ 즉시 적용:** 서명 요청 페이지 (서명자 이메일 입력)
- [x] **→ 즉시 적용:** 서명자 페이지 (`/sign/:token`)
  - [ ] ~~비회원 접근 가능~~ → QuSign 로그인 필요 (백엔드 OTP 엔드포인트 없음)
  - [x] QuSign 로그인 인증 (OTP 대체)
  - [ ] 문서 미리보기 (PDF.js 활용) ← 미완 (placeholder)
  - [x] 서명 버튼 (ML-DSA 개인키 복호화 비밀번호 입력 후 서명)
- [x] **→ 즉시 적용:** 무결성 검증 페이지
  - [ ] ~~PDF 드래그앤드롭 업로드~~ → 백엔드 파일 기반 검증 미지원, 토큰 검증으로 대체
  - [x] 검증 결과 표시 (서명자 / 서명 시각 / 변조 여부) — 실제 API 연동 완료
...
### 3-3.5. 유효성 검사 및 예외처리 강화

**목표:** 개발 중 발견된 미처리 예외·입력 검증 항목을 일괄 정리한다  
> 보안 리뷰(3-4) 전에 기본기를 잡는 단계

#### 완료된 항목 (개발 중 수정)

- [x] 파일 업로드 50MB 초과 — 프론트 클라이언트 사전 차단 + 백엔드 `MaxUploadSizeExceededException` → 413
- [x] 서명 비밀번호 오류 — `AEADBadTagException` → `InvalidSignaturePasswordException` → 400
- [x] 한글 파일명 다운로드 오류 — RFC 5987 `filename*=UTF-8''` 인코딩 적용
- [x] Spring multipart 기본 한도(1MB) → 50MB 상향

#### 백엔드 — 미처리 항목

- [ ] **회원가입 입력값 검증 강화**
  - [ ] 비밀번호 최소 길이 / 복잡도 규칙 (현재 `@NotBlank`만 존재)
  - [ ] 이메일 형식 서버사이드 검증 (현재 `@Email` 있으나 에러 메시지 한국어화 필요)
- [ ] **PDF 파일 형식 검증** — Content-Type 위조 방지 (실제 매직 바이트 확인)
- [ ] **서명 요청 중복 방지** — 동일 문서·서명자 조합 중복 요청 시 처리 정책 결정
- [ ] **만료된 서명 요청 정리** — 만료 토큰 재사용 시 명확한 에러 메시지 (현재 동작하나 검증 필요)
- [ ] **문서 소유권 검증 일관성** — 모든 문서 관련 API에서 소유자 체크 누락 여부 전수조사
- [ ] **서명 토큰 형식 검증** — 비정상 토큰 형식 입력 시 500 → 400으로 처리

#### 프론트엔드 — 미처리 항목

- [ ] **회원가입 비밀번호 규칙 안내** — 입력 중 실시간 유효성 피드백 (현재 없음)
- [ ] **서명 요청 페이지 이메일 중복 입력 방지** — 서명자 목록에 동일 이메일 2개 추가 가능한 상태
- [ ] **네트워크 에러 공통 처리** — axios 인터셉터에서 500/네트워크 오류 시 토스트 표시
- [ ] **토큰 만료 시 자동 로그아웃** — 401 응답 시 현재 무한 대기 가능성 확인
- [ ] **업로드 중 페이지 이탈 방지** — 해시 계산·업로드 도중 `beforeunload` 이벤트 처리
...
### 3-4. 보안 취약점 개선 (Claude Security 활용)

**목표:** 개발 완료 후 프론트엔드 + 백엔드 전 영역에 걸쳐 보안 취약점을 점검·수정한다

- [ ] **→ 즉시 적용:** `/security-review` 스킬로 현재 브랜치 전체 보안 리뷰 실행
- [ ] OWASP Top 10 항목별 점검
...
- [ ] 프론트엔드 XSS / CSRF 점검
- [ ] API 응답에서 민감 필드 노출 여부 점검 (개인키, 해시 원문 등)
- [ ] 취약점 발견 항목 이슈 등록 → 우선순위 분류 (Critical / High / Medium)
- [ ] Critical / High 항목 전부 수정
- [ ] 수정 후 재검토 및 `./gradlew test` 전체 통과 확인

**완료 기준:** Critical / High 취약점 0건, 보안 리뷰 결과 문서 보존
