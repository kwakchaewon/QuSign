...
- [ ] **1** [A] `/signup` → `requester@test.com` / `Test1234!` 가입 → 로그인 페이지 이동 확인
- [ ] **2** [B] `/signup` → `signer@test.com` / `Test1234!` 가입
- [ ] **3** [A] 로그인 → `/home` 진입 → 대시보드 통계 카드 모두 0 표시
- [ ] **4** [A] `서명 요청` → PDF 업로드 → 서명자 이메일 `signer@test.com` 입력 → 요청 생성 → 완료 화면에서 서명 링크 표시
- [ ] **5** [A] 헤더 벨 아이콘 → 배지 없음 확인 (아직 서명 전)
- [ ] **6** [B] 로그인 → 헤더 벨 아이콘 → 🔔 미읽음 1 "서명 요청" 알림 확인
