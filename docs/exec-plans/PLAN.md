...
- [ ] 서비스 컬러 팔레트 / 타이포그래피 정의
- [ ] 공통 컴포넌트 목록 정의 (버튼, 입력, 카드, 모달, 뱃지 등)
- [ ] **→ 즉시 적용:** Claude 디자인으로 핵심 화면 와이어프레임 생성
  - [ ] 로그인 / 회원가입 화면
  - [ ] 문서 목록 화면
  - [ ] PDF 업로드 / 서명 요청 화면
  - [ ] 서명자 화면 (`/sign/:token`)
  - [ ] 무결성 검증 화면
- [ ] **→ 즉시 적용:** Claude 디자인으로 UI 목업 → HTML/CSS 퍼블리싱 산출물 생성
- [ ] **→ 즉시 적용:** 디자인 토큰(CSS 변수) 정의 파일 작성
- [ ] 개발자 핸드오프 — 컴포넌트별 스펙 문서 확인
