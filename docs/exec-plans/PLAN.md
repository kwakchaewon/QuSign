...
- [ ] 비밀번호 최소 길이 / 복잡도 규칙 (`@NotBlank`만 존재)
- [ ] 이메일 형식 에러 메시지 한국어화
- [ ] PDF 파일 형식 검증 — 매직 바이트 확인 (Content-Type 위조 방지)
- [ ] 서명 요청 중복 방지 — 동일 문서·서명자 조합 처리 정책 결정
- [ ] 만료 토큰 재사용 시 명확한 에러 메시지 검증
- [ ] 서명 토큰 형식 비정상 입력 시 500 → 400 처리

#### 프론트엔드 — 미처리

- [ ] 회원가입 비밀번호 규칙 실시간 피드백
- [ ] 서명 요청 페이지 이메일 중복 입력 방지
- [ ] axios 인터셉터 — 500/네트워크 오류 시 토스트 표시
- [ ] 401 응답 시 자동 로그아웃
- [ ] 업로드 중 페이지 이탈 방지 (`beforeunload`)
...
## 5단계: AWS 배포 + SES + GitHub Actions
> 기간: 이후 / (구 4단계 + SES 이메일 연동 통합)
...
| 5개월 | Vue 3 프론트 구현 완성 | ✅ |
| 6개월 | 기능 고도화 & 품질 강화 완료 | ⬜ |
| 7개월 | AWS 배포 완료 + SES 이메일 + GitHub Actions | ⬜ |
