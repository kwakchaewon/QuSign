...
- [ ] **7** [B] 알림 클릭 → `/sign/{token}` 서명자 화면 진입 (미로그인 시 로그인 후 자동 이동)
- [ ] **8** [B] 문서 정보(요청자·만료일·SHA3-256) 확인 → PDF 미리보기 스크롤 → 동의 체크박스 2개 체크 → 계정 비밀번호 입력 → 서명하기 → "서명 완료" 화면
- [ ] **9** [A] 헤더 벨 아이콘 → 🔔 미읽음 1 "서명 완료" 알림 확인
- [ ] **10** [A] 알림 클릭 → 문서 상세 페이지 → 서명자 상태 `SIGNED` + 서명 일시 표시
- [ ] **11** [A] 서명된 PDF 다운로드 → `파일명_qusigned.pdf` 다운로드 확인
...
- [ ] **1** [A] 로그인 → `서명 요청` 진입
- [ ] **2** PDF 3개 드래그앤드롭 업로드 → 파일 목록 3행 표시, 각 파일 해시값 계산 확인
