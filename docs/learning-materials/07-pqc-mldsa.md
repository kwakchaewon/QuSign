...
**한 줄 요약**: RSA가 "인수분해"라는 어려운 수학 문제 하나에 안전성을 걸었던 것처럼, ML-DSA는 "격자에서 특정 점 찾기"라는 또 다른 어려운 수학 문제에 안전성을 겁니다. 이 문제의 핵심은 **양자컴퓨터로도 못 푼다**는 점입니다.

#### 격자(Lattice)란 — 비유로 먼저

**모눈종이를 떠올려보세요.** 격자는 이렇게 일정한 간격으로 반복되는 점들의 집합입니다.
...
**"차원이 높다"는 게 뭘 의미하나**: 위 모눈종이는 좌표가 `(x, y)` 두 개뿐인 2차원입니다. 격자 암호는 이 좌표축이 **수백~수천 개**인 공간을 씁니다. 개발자에게 익숙한 비유로 바꾸면, "컬럼이 2개짜리 테이블"이 아니라 "컬럼이 1000개짜리 초거대 테이블에서 특정 조건에 맞는 행을 찾는 것"과 비슷하다고 생각하면 됩니다.

**SVP (Shortest Vector Problem)** — 비유: 이 초거대 공간(격자) 안에서 "원점(0,0,...,0)에 가장 가까운 점"을 찾는 게임입니다. 좌표가 2~3개면 눈으로도 대충 찾을 수 있지만, 좌표가 수백~수천 개로 늘어나면 후보 경우의 수가 기하급수적으로 폭증해서, 사실상 무작위로 하나하나 다 뒤져보는 것 말고는 방법이 없어집니다 (지난 문서의 "체스판과 밀알" 비유와 같은 성격의 폭발입니다).

#### LWE (Learning With Errors) — "일부러 잡음을 섞은 방정식 풀기" 게임

일차방정식 여러 개를 주면 미지수(비밀값)를 쉽게 구할 수 있습니다.

```
2x + 3 = 11 → x = 4
```

그런데 방정식 결과에 매번 아주 작은 무작위 오차(노이즈)를 몰래 섞어서 알려준다면 어떨까요?

```
2x + 3 = 11.03   (원래는 11인데 +0.03 노이즈가 섞임)
2x + 3 = 10.98   (-0.02 노이즈)
2x + 3 = 11.05   (+0.05 노이즈)
```

방정식을 100개, 1000개 모아도 어느 게 "진짜 값"이고 어느 게 "노이즈"인지 구분할 수 없어서, x를 정확히 역산할 수 없습니다. **LWE는 이 원리를 수백 차원짜리 거대한 격자로 확장한 문제**입니다.
...
`aᵢ`, `bᵢ`는 공개되어도 되는 값이고 `s`가 비밀(개인키에 해당)입니다. `eᵢ`라는 "일부러 섞은 잡음" 때문에, 아무리 많은 `(aᵢ, bᵢ)` 쌍을 모아도 s를 역산할 수 없습니다. **ML-DSA는 이 LWE 문제를 좀 더 효율적으로 다듬은 버전(Module-LWE)을 사용합니다.**

#### 왜 양자컴퓨터로도 못 푸나 — "지름길이 아예 없는 미로"

Shor 알고리즘이 RSA를 깨는 원리는 "주기성이 있는 패턴을 양자적으로 빠르게 찾아내는 지름길"입니다 — 미로에 반복되는 패턴이 있으면 그 패턴을 이용해 한 번에 여러 갈래를 동시에 탐색하는 것과 비슷합니다. 그런데 **LWE는 애초에 이런 "반복되는 패턴(주기성)"이 없는 문제**라서, 양자컴퓨터의 이 지름길 자체를 쓸 수가 없습니다. 그래서 현재 가장 빠른 양자 알고리즘을 써도 여전히 지수 시간이 걸립니다 — 이게 ML-DSA가 "양자 시대에도 안전하다"고 하는 이유입니다.
...
**NIST (National Institute of Standards and Technology, 미국 국립표준기술연구소)**는 미국 상무부 산하 연방 기관으로, 도량형부터 IT 보안까지 폭넓은 분야의 표준을 제정합니다. 암호학계에서는 **사실상 최고 권위의 표준 제정 기관**으로 통합니다 — AES(대칭키 암호), SHA-2/SHA-3(해시 함수)도 NIST가 주관한 공개 공모전을 거쳐 선정된 표준입니다. PQC 표준(ML-KEM, ML-DSA 등) 역시 전 세계 암호학자들이 몇 년간 공개적으로 검증한 결과를 NIST가 최종 채택하는 방식으로 정해졌습니다.
...
### ML-DSA 내부 구조 — 서명이 실제로 만들어지는 과정

**먼저 큰 그림**: 웹 개발에서 익숙한 "challenge-response" 인증(nonce 기반 서명, OAuth PKCE 같은 것)과 비슷한 흐름입니다. 다만 상대방이 실시간으로 질문을 던지는 대신, **해시 함수가 그 "질문 역할"을 대신**합니다. 이런 방식을 **Fiat-Shamir 패러다임**이라고 부릅니다.

#### 비유: 물감 섞기 게임

서명자가 비밀 물감 `s`를 갖고 있고, 검증자는 그 물감으로 미리 만들어둔 결과물(`t = A·s`)만 압니다. 서명을 할 때마다 이런 과정을 거칩니다.

1. **임시 물감 y를 새로 준비** — 매번 랜덤하게, 이번 서명 한 번에만 씀 (마스킹 벡터)
2. **임시 물감의 결과(w = A·y)를 먼저 공개** — "나 이런 임시 색을 준비했어요"라고 미리 보여주는 것 (커밋먼트)
3. **메시지와 w를 해시해서 챌린지 값 c를 계산** — 원래는 상대방이 무작위 질문을 실시간으로 던져야 정상인데, 서명은 나중에 아무나 검증해야 하니 그 "질문 역할"을 해시 함수가 대신합니다 (이게 Fiat-Shamir 변환의 핵심 트릭)
4. **임시 물감과 비밀 물감을 challenge 비율로 섞어서 z = y + c·s를 제출** — 섞인 결과만 봐서는 원래 비밀 물감 s가 뭔지 알 수 없습니다. y가 s를 가려주는 마스크 역할을 합니다.
...
**검증이 어떻게 되나**: 검증자는 비밀 물감 s를 몰라도, 공개된 A·z와 c·t를 계산해서 그 차이가 서명자가 미리 공개했던 w와 일치하는지만 확인하면 됩니다 (위 식 마지막 줄). s를 전혀 모르는 상태에서도 "앞뒤가 수학적으로 맞는지"만 검산하는 게 이 방식의 핵심입니다.

#### "Aborts"가 필요한 이유 — 재시도 로직이라고 생각하면 쉽습니다

`z = y + c·s`에서 y가 s를 가려주는 마스크 역할을 하는데, 만약 z 값이 너무 크면 "y가 c·s를 충분히 가리지 못했다"는 신호가 되어 s에 대한 정보가 통계적으로 조금씩 새어나갑니다. 그래서 z가 안전한 범위를 벗어나면 이 시도는 그냥 버리고(abort), 새로운 y로 처음부터 다시 만듭니다 — 웹 개발에 빗대면 "조건에 안 맞는 응답이 나오면 폐기하고 재시도(retry)하는 로직"과 같은 발상입니다. 평균 4~7번 재시도하면 안전한 서명 하나가 완성됩니다.
