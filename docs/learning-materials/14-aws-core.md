...
EC2에 IAM 역할을 붙이면 `aws-sdk`가 자동으로 임시 자격증명을 획득합니다.
환경변수나 파일에 Access Key를 저장하지 않아도 됩니다.
...
> IAM 사용자는 Access Key/Secret Key로 인증합니다(사람이 사용). IAM 역할은 AWS 서비스가 다른 서비스에 접근할 때 사용합니다. EC2에 역할을 붙이면 EC2 내 코드가 Access Key 없이 임시 자격증명을 자동으로 획득합니다. 역할 기반 인증은 키가 만료돼도 자동 갱신되어 키 유출 위험이 없습니다.
...
> EventBridge Scheduler에서 시간대를 `Asia/Seoul`로 선택하면, cron 표현식도 그 시간대 기준으로 해석됩니다. UTC cron(`cron(30 12 * * ? *)`)을 그대로 쓰면 시간대가 이중 적용되어 실제로는 UTC+9+9=UTC+18 시각에 실행되는 버그가 발생합니다. 시간대를 설정했으면 cron 값도 그 시간대 기준으로 작성해야 합니다.
